...
- **M4 机翻基线 machine_text**（2026-09-03）：人改译文后无法回未改态/看不到机翻 → 根源是 translation 单字段被人工覆盖。加独立基线列 `machine_text`（最近一次 AI 输出）：AI 落库即记基线；人改只动 translation 基线保留（编辑弹窗「机翻原文」可看 + 一键恢复，edited 归 0）；清空回待译（translation=null）基线一并清掉，状态机放行单次 **{MACHINE,EDITED}→PENDING**（CONFIRMED 仍不可逆）——EDITED(3) 一并放行：M4 基线改造前落库的旧条目状态就是 3，清空旧已改条目实测报「非法状态迁移: 3 -> 1」（2→1 放行后仍失败），补 3→1 修复。协议 Entry 加字段 → `pnpm protocol`；migration 004（schema 3→4 自动 backfill）。核心 pytest/ruff/mypy + 前端 typecheck/lint/build 全绿
- **M4 侧边栏状态置顶 + 文件过滤**（2026-09-03）：用户痛点「文件几十个，状态筛选被推出滚动折叠线外，每次切状态/文件都得滚到底」。改 [Sidebar.tsx](apps/desktop/src/components/Sidebar.tsx)：状态行钉顶固定（栏 overflow:hidden）、文件区独立滚动 + 即时过滤框（子串匹配，检索代替滚动）、文件角标随状态变工作队列（选中「待译」后角标=该文件剩几条，有活琥珀、0 置灰）。纯前端 vite 热更，无需 sidecar。前端四 gate 全绿
- **M4 片段/文件勾选翻译范围**（2026-09-03）：默认翻全部，用户要能挑文件/片段翻。store 加 `selectedIds`（Set 每次整换）+ `toggleEntry/setFileSelection/setVisibleSelection/clearSelection`；`startTranslate` 有勾选 → `scope:"selection"+ids`，否则维持 `scope:"all"`（后端 `_select_translatable` 本就支持 selection，只挑其中未翻译+非已确认，前端只改壳）。UI：表格首列行首方块勾片段、表头方块三态勾当前筛选；[Sidebar](apps/desktop/src/components/Sidebar.tsx) 文件行前三态勾——**文件全选=其所有片段都勾**（按全量算，不受状态筛选影响）；[TranslateModal](apps/desktop/src/components/TranslateModal.tsx) 区分「翻译所选 X 条 / 全部」+ 清除勾选。三态组件 [Checkbox.tsx](apps/desktop/src/components/Checkbox.tsx)。纯前端 vite 热更
- **M4 翻译引擎启用选择器**（2026-09-03）：store 早有 selectedProvider/selectedModel 但**无 UI 调用**，translate.start 只传 provider_id、模型永远走后端默认第一个——多服务形同虚设。改 [StatusBar.tsx](apps/desktop/src/components/StatusBar.tsx) 右端加 CC-switch 式引擎 pill + drop-up 面板（fixed 锚 viewport，绕 WebView2 absolute 弹层被 Modal overflow 裁剪坑；外层 mousedown/Escape 关），点某服务下某模型即 `store.enableEngine` 启用；未配置时面板内引导去模型 API。`startTranslate` 补传 `model`（协议本就支持，只加不改）。纯前端 vite 热更
- **M4 翻译提速/提质**（2026-09-03）：用户「翻 2061 条太慢」。瓶颈=API 串行生成输出 token，杠杆只有并发×减少重复×模型档。落地：① **同源去重**——批内 source 相同条目只请求一次，结果复制组内各条（保护器确定性→同源 protected/tokens 一致；防御非确定性保护器→同源 protected 不一致拆独立组照常各请求）；② **翻译缓存 translate_cache 启用**（M1 建表 M3 未接线）——key=sha1(provider+model+术语表版本+**prompt 版本 _CACHE_PROMPT_VERSION**+source)，同条件重跑/断点续翻 0 成本；换模型/改术语/改提示词自动失效；「保留原文」兜底结果不入缓存（否则永远不再重试）；**translate.retranslate_entries 强制 cache_scope=None**（重翻吃缓存=返回上次坏译文，白翻）；③ **few-shot/说话人接线**——fill_speaker_and_few_shot（M3 写了没调用）组批后填充，provider 契约加 few_shot/speaker 参数注入 system prompt（同文件已确认译文当风格示例+角色名→术语语气一致，每批成本=几十 token 输入）；④ **并发 2→4**（真机仍限流可设环境变量 GAMETR_TRANSLATE_CONCURRENCY 调小）+ **429 排队化**——openai_compat 尊重 Retry-After 退避（上限 60s），限流转为等待而非丢批。核心 pytest/ruff/mypy 全绿，sidecar 需重打包
...
3. **M3 遗留**：提示词评测集（Phase 6，100 条黄金样本+人工译文）；断点续翻 kill -9 集成测试；resume 不重启已停止协程（需重新 start）
